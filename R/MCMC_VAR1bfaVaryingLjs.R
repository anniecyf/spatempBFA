...
#'
#' @return \code{VAR1bfaVaryingLjs} returns a list containing the following objects (some may be NULL)
#'
#'   \describe{
#'
...
#'
#' @export
VAR1bfaVaryingLjs <- function(formula, data, dist, Nu, K, LjVec, 
                   ...
                   spatApprox = TRUE, alphaSequen = FALSE, h = 15,
  ...
  ###Check model inputs
  VAR1CheckInputsVaryingLjs(formula, data, dist, Nu, K, LjVec, starting, hypers, tuning, 
  ...
  DatObj <- VAR1CreateDatObjVaryingLjs(formula, data, dist, Nu, K, family, spatial.structure, 
                         gamma.shrinkage, include.space, spatApprox, alphaSequen, h, 
                         storeSpatPredPara, storeWeights)
  HyPara <- VAR1CreateHyPara(hypers, DatObj) 
  MetrObj <- VAR1CreateMetrObj(tuning, DatObj)
  Para <- VAR1CreateParaVaryingLjs(starting, DatObj, HyPara, LjVec)
  ...
  RawSamples <- VAR1CreateStorageVaryingLjs(DatObj, McmcObj)
  
  ###Time MCMC sampler
  BeginTime <- Sys.time()

  ###Run MCMC sampler in Rcpp
  RegObj <- VAR1bfaRcppVaryingLjs(DatObj, HyPara, MetrObj, Para, SpatPara, DatAug, McmcObj, RawSamples, Interactive)
  ...
  Samples <- VAR1FormatSamplesVaryingLjs(DatObj, RegObj$rawsamples)

  ###Return VAR1varyLjBFA object
  VAR1varyLjBFA <- list(lambda = Samples$Lambda,
                eta = Samples$Eta,
                beta = Samples$Beta,
  ...
  VAR1varyLjBFA <- structure(VAR1varyLjBFA, class = "VAR1varyLjBFA")
  return(VAR1varyLjBFA)
