...
#'
#' \code{bfaFixedL} is a Markov chain Monte Carlo (MCMC) sampler for a Bayesian spatial factor analysis model. The spatial component is 
...
#' 
#'
#' @return \code{bfaFixedL} returns a list containing the following objects (some may be NULL)
#'
#'   \describe{
#'
...
#'
#' @export
bfaFixedL <- function(formula, data, dist, time, K, L = 20, 
                   ...
                   gamma.shrinkage = TRUE, include.time = TRUE, include.space = TRUE, clustering = TRUE, 
                   seasonPeriod = 1, equalTimeDist = TRUE, spatApprox = TRUE, alphaMethod = "block", h = 15,
  ...
  ###Check model inputs
  CheckInputsFixedL(formula, data, dist, time, K, L, starting, hypers, tuning, 
              mcmc, family, temporal.structure, spatial.structure, 
              gamma.shrinkage, include.time, include.space, clustering,
              seasonPeriod, equalTimeDist, spatApprox, alphaMethod, h,
  ...
  DatObj <- CreateDatObjFixedL(formula, data, dist, time, K, L, family, 
  ...
  Para <- CreateParaFixedL(starting, DatObj, HyPara)
  ...
  RawSamples <- CreateStorageFixedL(DatObj, McmcObj)
  
  ###Time MCMC sampler
  BeginTime <- Sys.time()

  ###Run MCMC sampler in Rcpp
  RegObj <- bfaRcppFixedL(DatObj, HyPara, MetrObj, Para, ParaCL, SpatPara, DatAug, McmcObj, RawSamples, Interactive)
  ...
  FixedLbfa <- structure(FixedLbfa, class = "FixedLbfa")
  return(FixedLbfa)
