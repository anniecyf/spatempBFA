CheckInputsFixedL <- function(formula, data, dist, time, K, L, starting, hypers, tuning, mcmc, family, temporal.structure, spatial.structure, 
                        gamma.shrinkage, include.time, include.space, clustering, seasonPeriod, equalTimeDist, spatApprox, alphaMethod, 
  ...
  if (!is.logical(include.time)) stop('"include.time" must be a logical')
  ...
  if (!is.logical(equalTimeDist)) stop('"equalTimeDist" must be a logical')
  if (!is.logical(spatApprox)) stop('"spatApprox" must be a logical')
  if (!is.logical(storeSpatPredPara)) stop('"storeSpatPredPara" must be a logical')
  if (!is.logical(storeWeights)) stop('"storeWeights" must be a logical')
  if (!is.logical(alphasWeightsToFiles)) stop('"alphasWeightsToFiles" must be a logical')
  if (!clustering & storeSpatPredPara) stop('storeSpatPredPara cannot be TRUE without clustering')
  if (!clustering & storeWeights) stop('storeWeights cannot be TRUE without clustering')
  if (!clustering & gamma.shrinkage) stop('gamma.shrinkage prior cannot be used without clustering')
  if (!clustering & alphasWeightsToFiles) stop('alphasWeightsToFiles cannot be TRUE without clustering')
  if (alphasWeightsToFiles & !storeSpatPredPara & !storeWeights) stop('alphasWeightsToFiles cannot be TRUE if both storeSpatPredPara and storeWeights are FALSE')

  ###Family
  if ((length(family) != O) & (length(family) != 1)) stop(paste0('family: must have 1 or O = ', O, ' entries'))
  if (!all(family %in% c("normal", "probit", "tobit", "binomial"))) stop('family: All entries must be "normal", "probit", "tobit", or "binomial"')
  
  ###Temporal correlation
  if (!temporal.structure %in% c("ar1", "exponential", "sar1", "sexponential")) stop('temporal.structure: must be one of "ar1", "exponential", "sar1" or"sexponential"')
  if (!is.wholenumber(seasonPeriod) | seasonPeriod < 0) stop('mcmc: "seasonPeriod" must be a positive integer')
  
  ###Spatial correlation
  if (!spatial.structure %in% c("discrete", "continuous")) stop('spatial.structure: must be one of "discrete" or "continuous"')
  if (!alphaMethod %in% c("block", "sequential")) stop('alphaMethod: must be one of "block" or "sequential"')
  if (spatApprox & (spatial.structure != "continuous")) stop('spatial.structure must be "continuous" when spatApprox is TRUE')
  if (!is.wholenumber(h) | h <= 0) stop('h must be a strictly positive integer')
  if (h >= M) stop('h must be strictly smaller than the number of location points')
  ...
  if (!("trials" %in% names(data)) & ("binomial" %in% family)) stop('trials must be specified as a variable in the data frame data if there are observation type(s) from family "binomial"')
  if ("trials" %in% names(data)) {
    Trials <- data$trials 
    if (!("binomial" %in% family)) stop('the data frame data should not contain a variable trials without any observation type from family = "binomial"')
    if (length(Trials) != N) stop(paste0('data$trials must have exactly ', N, ' values')) # I check dimensions later
    if (any(is.na(Trials))) stop("data$trials may have no missing values")
    if (any(!is.finite(Trials))) stop("data$trials must have strictly finite entries")
    if (any(Trials != floor(Trials))) stop("data$trials must have integers only")
    if (any(Trials < 1)) stop("data$trials must contain positive integers only")
  }
  ...
  outcome <- all.vars(formula)[1] # outcome variable "Y"
  ...
  if (any(familyInd == "binomial")) {
    ...
    for (o in 1:O) {
      if (familyInd[o] == "binomial") {
        ...
        if (any(YArray[, o, ] != floor(YArray[ , o, ]))) stop('Y: for "binomial" observed data must be non-negative integers')
        if (any(YArray[, o, ] > TrialsArray[, o, ])) stop('Y: for "binomial" observed data must be less than or equal to the corresponding number of trials')
      }
    }
  } 
  ...
  if (dim(dist)[1] != M) stop(paste0('dist must be an ', M ,' x ', M, ' dimensional matrix'))
  if (dim(dist)[2] != M) stop('dist must be a square matrix')
  if (sum(!((dist) == t(dist))) > 0) stop('dist must be symmetric')
  if (any(diag(dist) != 0)) stop('dist must have only zeros on the diagonal')
  if (any(is.na(dist))) stop('dist cannot have missing values')
  ...
  if (any(time < 0)) stop('time vector has at least one negative point')
  ...
  if (!is.wholenumber(L) | L <= 0 | is.infinite(L)) stop('L must be a strictly positive integer')
  if (clustering & L == 1) stop('L must be bigger than 1 when clustering = TRUE') 
  
  ###hypers
  if (!is.null(hypers)) {
    if (!is.list(hypers)) stop('hypers must be a list')
    if (!all(names(hypers) %in% c("Sigma2", "Kappa", "Rho", "Delta", "Beta", "Psi", "Upsilon"))) stop('hypers: Can only contain lists with names "Sigma2", "Kappa", "Rho", "Delta", "Beta", "Psi", or "Upsilon"')
    ...
    if ("Sigma2" %in% names(hypers)) {
      if (any(family %in% c("normal", "probit", "tobit"))) {
        ...
        stop('hypers: "Sigma2" cannot be included for "binomial" likelihood')
      }
    }
    ...
    if ("Kappa" %in% names(hypers)) {
      ...
      if (O > 1) {
        ...
        if (!dim(hypers$Kappa$BigTheta)[1] == O) stop('hypers: "BigTheta" must be of dimension O by O')
        if (any(is.na(hypers$Kappa$BigTheta))) stop('hypers: "BigTheta" cannot have missing values')
        if (!all(is.finite(hypers$Kappa$BigTheta))) stop('hypers: "BigTheta" cannot have infinite values')
        if (dim(hypers$Kappa$BigTheta)[2] != O) stop('hypers: "BigTheta" must be a square matrix')
        if ( any(hypers$Kappa$BigTheta) != t(hypers$Kappa$BigTheta) ) stop('hypers: "BigTheta" must be symmetric')
    ...
    if ("Rho" %in% names(hypers)) {
      if (spatial.structure == "discrete") {
        if (!is.null(hypers$Rho)) stop('hypers: When spatial.structure = "discrete", "Rho" must be missing or NULL')
      } else { #      if (spatial.structure == "continuous") 
    ...
    ###If Beta hyperparameters are provided
    if ("Beta" %in% names(hypers)) {
      if (P == 0) stop('hypers: "Beta" should not be provided when there are no coviariate(s)')
      if (!is.list(hypers$Beta)) stop('hypers: "Beta" must be a list')
      if (!"MuBeta" %in% names(hypers$Beta)) stop('hypers: "MuBeta" value missing')
      if (length(hypers$Beta$MuBeta) != P) stop('hypers: "MuBeta" must be a vector of length P')
      if (any(is.na(hypers$Beta$MuBeta))) stop('hypers: "MuBeta" cannot contain NA value(s)')
      if (any(!is.finite(hypers$Beta$MuBeta))) stop('hypers: "MuBeta" cannot contain infinite value(s)')
      if (!"SigmaBeta" %in% names(hypers$Beta)) stop('hypers: "SigmaBeta" value missing')
      if (P == 1){
        if (!is.scalar(hypers$Beta$SigmaBeta)) stop('hypers: "SigmaBeta" must be a scalar when P = 1')
        if (is.na(hypers$Beta$SigmaBeta)) stop('hypers: "SigmaBeta" cannot be NA')
        if (!is.finite(hypers$Beta$SigmaBeta)) stop('hypers: "SigmaBeta" cannot be infinite')
        if (hypers$Beta$SigmaBeta <= 0) stop('hypers: "SigmaBeta" must be strictly positive')
      } else { #if (P > 1)
        if (!is.matrix(hypers$Beta$SigmaBeta)) stop('hypers: "SigmaBeta" must be a matrix')
        if (dim(hypers$Beta$SigmaBeta)[1] != P) stop('hypers: "SigmaBeta" must be of dimension P by P')
        if (any(is.na(hypers$Beta$SigmaBeta))) stop('hypers: "SigmaBeta" cannot have missing values')
        if (!all(is.finite(hypers$Beta$SigmaBeta))) stop('hypers: "SigmaBeta" cannot have infinite values')
        if (dim(hypers$Beta$SigmaBeta)[2] != P) stop('hypers: "SigmaBeta" must be a square matrix')
        if ( any( (hypers$Beta$SigmaBeta) != t(hypers$Beta$SigmaBeta) ) ) stop('hypers: "SigmaBeta" must be symmetric')
        if ((det(hypers$Beta$SigmaBeta) - 0) < 0.00001) stop('hypers: "SigmaBeta" is close to singular')
      }      
    }
    ...
    if ("Upsilon" %in% names(hypers)) {
      ...
      if (K > 1) {
        ...
        if (any(is.na(hypers$Upsilon$Omega))) stop('hypers: "Omega" cannot have missing values')
        ...
        if ( any( (hypers$Upsilon$Omega) != t(hypers$Upsilon$Omega) ) ) stop('hypers: "Omega" must be symmetric')
    ...
    if ("Psi" %in% names(hypers)) {
      ...
      if (temporal.structure %in% c("exponential", "sexponential")) {
      ...
      } else { #if (temporal.structure %in% c("ar1", "sar1")) 
        if (!"APsi" %in% names(hypers$Psi)) stop('hypers: "APsi" value missing')
        if (!is.scalar(hypers$Psi$APsi)) stop('hypers: "APsi" must be a scalar')
        if (is.na(hypers$Psi$APsi)) stop('hypers: "APsi" cannot be NA')
        if (!is.finite(hypers$Psi$APsi)) stop('hypers: "APsi" cannot be infinite')
  ...
  if (!is.null(starting)) {
    ...
    if (!all(names(starting) %in% c("Sigma2", "Kappa", "Rho", "Delta", "Psi", "Upsilon", "Beta"))) stop('starting: Can only contain objects with names "Sigma2", "Kappa", "Rho", "Delta", "Beta", "Psi", and "Upsilon"')
    ...
    if ("Delta" %in% names(starting)) {
      ...
      if (is.vector(starting$Delta)) {
        if (!is.numeric(starting$Delta)) stop('starting: "Delta" must be a numeric vector')
      ...
      } else if (is.scalar(starting$Delta)) {
    ...
    if ("Beta" %in% names(starting)) {
      ...
      if (is.vector(starting$Beta)) {
        if (!is.numeric(starting$Beta)) stop('starting: "Beta" must be a numeric vector')
        if (length(starting$Beta) != P) stop('starting: "Beta" must be length P')
        if (any(is.na(starting$Beta))) stop('starting: "Beta" cannot have missing values')
        if (!all(is.finite(starting$Beta))) stop('starting: "Beta" cannot have infinite values')
      } else if (is.scalar(starting$Beta)) {
    ...
    if ("Sigma2" %in% names(starting)) {
      if (any(family %in% c("normal", "probit", "tobit"))) {
        ...
        if (is.matrix(starting$Sigma2)) {
          ...
          if (any(is.na(starting$Sigma2))) stop('starting: "Sigma2" cannot have missing values')
          if (!all(is.finite(starting$Sigma2))) stop('starting: "Sigma2" cannot have infinite values')
          if (any(starting$Sigma2 <= 0)) stop('starting: all values in "Sigma2" must be strictly positive')
        } else if (is.scalar(starting$Sigma2)) {
          ...
          if (starting$Sigma2 <= 0) stop('starting: "Sigma2" must be positive')
        }        
    ...
    if ("Kappa" %in% names(starting)) {
      if (O > 1) {
        ...
        if (dim(starting$Kappa)[1] != O) stop('starting: "Kappa" must be of dimension O by O')
        if (dim(starting$Kappa)[2] != O) stop('starting: "Kappa" must be a square matrix')
        if (any(is.na(starting$Kappa))) stop('starting: "Kappa" cannot have missing values')
        if (!all(is.finite(starting$Kappa))) stop('starting: "Kappa" cannot have infinite values')
        if ( any( (starting$Kappa) != t(starting$Kappa) ) ) stop('starting: "Kappa" must be symmetric')
    ...
    if ("Upsilon" %in% names(starting)) {
      if (K > 1) {
        ...
        if (dim(starting$Upsilon)[1] != K) stop('starting: "Upsilon" must be of dimension K by K')
        if (dim(starting$Upsilon)[2] != K) stop('starting: "Upsilon" must be a square matrix')
        if (any(is.na(starting$Upsilon))) stop('starting: "Upsilon" cannot have missing values')
        if (!all(is.finite(starting$Upsilon))) stop('starting: "Upsilon" cannot have infinite values')
        if ( any( (starting$Upsilon) != t(starting$Upsilon) ) ) stop('starting: "Upsilon" must be symmetric')
...
VAR1CheckInputsFixedL <- function(formula, data, dist, Nu, K, L, starting, hypers, tuning, 
  ...
  if (!is.null(starting)) {
    if (!is.list(starting)) stop('starting must be a list')
    if (!all(names(starting) %in% c("Sigma2", "Kappa", "Rho", "Delta", "A", "Upsilon", "Beta"))) stop('starting: Can only contain objects with names "Sigma2", "Kappa", "Rho", "Delta", "Beta", "A", and "Upsilon"')
    ...
    ###If A starting values is provided
    if ("A" %in% names(starting)) {
      if (K > 1) {
        if (!is.matrix(starting$A)) stop('starting: "A" must be a matrix')
        if (dim(starting$A)[1] != K) stop('starting: "A" must be of dimension K by K')
        if (dim(starting$A)[2] != K) stop('starting: "A" must be a square matrix')
        if (any(is.na(starting$A))) stop('starting: "A" cannot have missing values')
        if (!all(is.finite(starting$A))) stop('starting: "A" cannot have infinite values')
        if ( any( (starting$A) != t(starting$A) ) ) stop('starting: "A" must be symmetric')
        if ((det(starting$A) - 0) < 0.000000000000001) stop('starting: "A" is close to singular')
      } else { #if (K == 1) 
        if (!is.scalar(starting$A)) stop('starting: "A" must be a scalar when K = 1')
        if (is.na(starting$A)) stop('starting: "A" cannot be NA')
        if (!is.finite(starting$A)) stop('starting: "A" cannot be infinite')
      }
    }
  ...
  if (!is.null(tuning)) {
    if (!is.list(tuning)) stop('tuning must be a list')
    if (any(names(tuning) != "Rho")) stop('tuning: Can only contain objects with name "Rho"')
...
CheckInputsVaryingLjs <- function(formula, data, dist, time, K, LjVec, starting, hypers, tuning, mcmc, family, temporal.structure, spatial.structure, 
                        gamma.shrinkage, include.time, include.space, seasonPeriod, equalTimeDist, spatApprox, alphaSequen, h, storeSpatPredPara, storeWeights) {
  ...
  if (!is.logical(equalTimeDist)) stop('"equalTimeDist" must be a logical')
  if (!is.logical(spatApprox)) stop('"spatApprox" must be a logical')
  if (!is.logical(alphaSequen)) stop('"alphaSequen" must be a logical')
  if (!is.logical(storeSpatPredPara)) stop('"storeSpatPredPara" must be a logical')
  if (!is.logical(storeWeights)) stop('"storeWeights" must be a logical')
  ...
  if (dim(dist)[1] != M) stop(paste0('dist must be an ', M ,' x ', M, ' dimensional matrix'))
  if (dim(dist)[2] != M) stop('dist must be a square matrix')
  if (any((dist) != t(dist)))  stop('dist must be symmetric')
  if (any(diag(dist) != 0)) stop('dist must have only zeros on the diagonal')
  if (any(is.na(dist))) stop('dist cannot have missing values')
  if (!all(is.finite(dist))) stop('dist cannot have infinite values')
  if (spatial.structure == "discrete") if (length(table(dist)) > 2) stop('dist must only contain binaries (i.e. 0\'s or 1\'s) for "discrete" space')
  ...
  ###Data checks for LjVec
  if (length(LjVec) != K) stop('LjVec must be of length K')
  bool <- logical(length = K)
  for (i in 1:K) bool[i] <- (is.na(LjVec[i]) | !is.wholenumber(LjVec[i]) | LjVec[i] <= 0 | is.infinite(LjVec[i])) 
  if (any(bool)) stop('LjVec: All entries must be positive integers')
  ...
  if (!is.null(starting)) {
    ...
    if ("Delta" %in% names(starting)) {
      ...
      if (is.vector(starting$Delta)) {
        if (!is.numeric(starting$Delta)) stop('starting: "Delta" must be a numeric vector')
        if (length(starting$Delta) != K) stop('starting: "Delta" must be length K')
        if (any(is.na(starting$Delta))) stop('starting: "Delta" cannot have missing values')
        if (!all(is.finite(starting$Delta))) stop('starting: "Delta" cannot have infinite values')
        if (any(starting$Delta < 0)) stop('starting: "Delta" cannot have non-negative values')
      } else if (is.scalar(starting$Delta)) {
        if (is.na(starting$Delta)) stop('starting: "Delta" cannot be NA')
...
VAR1CheckInputsVaryingLjs <- function(formula, data, dist, Nu, K, LjVec, starting, hypers, tuning, 
                                                       mcmc, family, spatial.structure, gamma.shrinkage, include.space,
                                                       spatApprox, alphaSequen, h, storeSpatPredPara, storeWeights) {
