###Function to get model fit diagnostics given a FixedLbfa object
#'
#' predictNewLocFixedL
#'
#' Predicts observations at new location point(s) from a \code{\link{FixedLbfa}} or \code{\link{FixedLbfaVAR1}} model.
#'
#' @param object A \code{\link{FixedLbfa}} model object for which predictions
#'  are desired from.
#'
...
#'
#' @details \code{predictNewLocFixedL} predicts vectors at new spatial location(s).
...
#'
#' @return \code{predictNewLocFixedL} returns a list containing the following objects.
#'
#'   \describe{
#'
...
predictNewLocFixedL <- function(object, NNewLoc, distOrigNew = NULL, distNewNew = NULL, NewX = NULL, NewTrials = NULL, Verbose = TRUE, seed = 27) {
  ...
  if ((CL == 1) & (DatObj$spatPred == 0)) stop("If the Bayesian non-parametric process is used when fitting the model, then 'storeSpatPredPara' must be TRUE to enable predictions at new spatial locations")
  if (IS == 1) {
  ...
  if (DatObj$alphaWsFiles == 0) {Para$Alpha <- object$alpha} else {Para$Alpha <- matrix(1,1,1)}
