...
arma::colvec GetLogLik(Rcpp::List DatObj_List, Rcpp::List Para_List, int NKeep, bool Verbose) {
    ...
    int YSInt, TrialInt;
    YCube(arma::span::all, arma::span(0, 0), arma::span(0, 0)) = YStar;
    ...
    for (int s = 0; s < NKeep; s++) {
        ...
        for (arma::uword f = 0; f < O; f++) {
            ...
            if (FamilyType == 0) {
                ...
                arma::colvec VarVec = Sigma2.col(count);
            ...
            if (FamilyType == 1) {
                arma::mat Sigma2 = arma::reshape(Sigma2Mat.row(s), M, (O - C));
                arma::colvec SDVec = arma::sqrt(Sigma2.col(count));
                ...
                for (int i = 0; i < (M * Nu); i++) {
                    ...
                    else LogLiks += arma::as_scalar(arma::log(1 - PNormS)); // if (YS == 1)
                }
            }
            ...
            if (FamilyType == 2) {
                ...
                for (int i = 0; i < (M * Nu); i++) {
                    ...
                    PNormS(0) = pnormRcpp(-MeanS / SDS);
                    if (YS == 0) LogLiks += arma::as_scalar(arma::log(PNormS));
                    else LogLiks += dlnorm(YS, MeanS, SDS * SDS); //if (YS > 0)
                }
            }
...
double GetLogLikMean(Rcpp::List DatObj_List, Rcpp::List Para_List) {
    ...
    for (arma::uword f = 0; f < O; f++) {
        ...
        if (FamilyType == 0) LogLikMean += arma::sum(dlnormRcpp(YVec, MuVec, CovVec));
        else if (FamilyType == 1) { //Probit
            for (int i = 0; i < (M * Nu); i++) {
                SDVec = arma::sqrt(CovVec);
                YS = YVec(i);
                MuS = MuVec(i);
                SDS = SDVec(i);
                PNormS(0) = pnormRcpp(-MuS / SDS);
                if (YS == 0) LogLikMean += arma::as_scalar(arma::log(PNormS));
                else LogLikMean += arma::as_scalar(arma::log(1 - PNormS)); // if YS == 1
            }
        }
            ...
            for (int i = 0; i < (M * Nu); i++) {
                ...
                PNormS(0) = pnormRcpp(-MuS / SDS);
                if (YS == 0) LogLikMean += arma::as_scalar(arma::log(PNormS));
                else LogLikMean += dlnorm(YS, MuS, SDS * SDS); // if YS > 0
            }
        }
        else if (FamilyType == 3) { //Categorical
