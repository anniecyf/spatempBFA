...
save(regFixedL.simu, file="regFixedL60simuT30M900Iter200.RData")
library(coda)
Diags <- diagnostics(regFixedL.simu, diags = c("dic", "dinf", "meanIC", "waic"), keepDeviance = TRUE)
save(Diags, file = "regFixedL60simuT30M900Iter200Diags.RData")
Deviance <- as.mcmc(Diags$deviance)
save(Deviance, file = "regFixedL60simuT30M900Iter200Deviance.RData")
...
save(regFixedL.simu.block, file="regFixedL60simuBlockT30M900Iter200.RData")
library(coda)
Diags.block <- diagnostics(regFixedL.simu.block, diags = c("dic", "dinf", "meanIC", "waic"), keepDeviance = TRUE)
save(Diags.block, file = "regFixedL60simuBlockT30M900Iter200Diags.RData")
Deviance.block <- as.mcmc(Diags.block$deviance)
save(Deviance.block, file = "regFixedL60simuBlockT30M900Iter200Deviance.RData")
...
regFixedL.simu.sequen <- bfaFixedL(Y ~ 0, data = dat, dist = D, time = Time,  K = K, 
                                   ...
                                   alphasWeightsToFiles = FALSE) 
Sys.time()
save(regFixedL.simu.sequen, file="regFixedL60simuSequenT30M900Iter200nostorealphaweights.RData")
...
save(Diags.sequen, file = "regFixedL60simuSequenT30M900Iter200DiagsNostorealphaweights.RData")
Deviance.sequen <- as.mcmc(Diags.sequen$deviance)
save(Deviance.sequen, file = "regFixedL60simuSequenT30M900Iter200DevianceNostorealphaweights.RData")
...
regVaryLj.simu.sequen <- bfaVaryingLjs(Y ~ 0, data = dat, dist = D, time = Time,  K = K, LjVec = LjVec,
                                       ...
                                       storeWeights = FALSE) 
Sys.time()
save(regVaryLj.simu.sequen, file="regVaryLjsimuSequenT30M900Iter200nostorealphaweight.RData")
...
save(Diags.sequenVaryLj, file = "regVaryLjsimuSequenT30M900Iter200DiagsNostorealphaweight.RData")
