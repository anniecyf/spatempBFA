...
reg.simu <- bfa_sp(Y ~ 0, data = dat, dist = D, time = Time,  K = K, 
                  ...
                  include.space = TRUE,
                  clustering = TRUE) 
save(reg.simu, file="spBFAL60simuT30M100Iter200.RData")
Sys.time()
reg.simu$runtime
library(coda)
Diags <- spBFA::diagnostics(reg.simu, diags = c("dic", "dinf", "waic"), keepDeviance = TRUE)
save(Diags, file="spBFAL60simuT30M100Iter200Diags.RData")
Deviance <- as.mcmc(Diags$deviance)
traceplot(Deviance, ylab = "Deviance", main = "Posterior Deviance")
#save(Deviance, file = "spBFAL60simuT30M100Iter200Deviance.RData")
...
reg.simu.LInf <- bfa_sp(Y ~ 0, data = dat, dist = D, time = Time,  K = K, 
                   ...
                   include.space = TRUE,
                   clustering = TRUE) 
save(reg.simu.LInf, file="spBFALInfsimuT30M100Iter200.RData")
Sys.time()
reg.simu.LInf$runtime
library(coda)
Diags.LInf <- spBFA::diagnostics(reg.simu.LInf, diags = c("dic", "dinf", "waic"), keepDeviance = TRUE)
save(Diags.LInf, file="spBFALInfsimuT30M100Iter200Diags.RData")
Deviance.LInf <- as.mcmc(Diags.LInf$deviance)
traceplot(Deviance.LInf, ylab = "Deviance", main = "Posterior Deviance")
#save(Deviance, file = "spBFALInfsimuT30M100Iter200Deviance.RData")
