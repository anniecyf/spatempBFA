...
reg.simu.LInf <- bfa_sp(Y ~ 0, data = dat, dist = D, time = Time,  K = K, 
                        ...
                        include.space = TRUE,
                        clustering = TRUE) 
save(reg.simu.LInf, file="spBFALInfsimuT30M900Iter30000.RData")
Sys.time()
reg.simu.LInf$runtime
library(coda)
Diags.LInf <- spBFA::diagnostics(reg.simu.LInf, diags = c("dic", "dinf", "waic"), keepDeviance = TRUE)
save(Diags.LInf, file="spBFALInfsimuT30M900Iter30000Diags.RData")
Deviance.LInf <- as.mcmc(Diags.LInf$deviance)
#save(Deviance, file = "spBFALInfsimuT30M900Iter30000Deviance.RData")
