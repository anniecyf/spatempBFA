...
trainingTestingPlotBW <- ggplot(spatGpDF) + ggtitle("Training / Testing Spatial Points") +
  geom_point(aes(x = x, y = y, col = trainingTestingSpatGp), size = 7, show.legend = FALSE) +
...
actualSpatPlotBW <- ggplot(spatGpDF) + ggtitle("2 Actual Spatial Groups") + 
  geom_point(aes(x = x, y = y, col = actualGp), size = 7, show.legend = FALSE) + 
...
ggarrange(trainingTestingPlotBW, actualSpatPlotBW, align = "h", ncol = 2, nrow = 1, 
          labels = c("A", "B"))
```
The entire simulated data set consists of $y_t(\bm{s}_i)$'s from `r Nu` time points and `r M` spatial locations, which belong to 2 actual spatial groups (**B**). `r m` out of the `r M` locations (**A**) are training ones, and the 9 remaining black locations (**A**) are testing ones. The training data set used to fit all 6 models (our 4 models `fullGPfixedL`, `NNGPblockFixedL`, `NNGPsequenFixedL`, and `NNGPsequenVaryLj` $\&$ `spBFAL10`, `spBFALInf` from the R package `spBFA`) consists of data at the `r m` white locations (**A**) and the first `r trainingT` time points. $y_t(\bm{s}_i)$'s at the `r M-m` testing locations for the first `r trainingT` time points are used to assess our 4 models' spatial prediction performances. Similarly, $y_t(\bm{s}_i)$'s at the `r m` training locations for the last `r testingT` time points are used to assess all 6 models' temporal prediction performances.
...
library(spBFA) # to compare overall runtime, Diags, Deviances, and future time prediction results
...
setwd("..")
```
As expected, our 4 models are a few times faster than the 2 `spBFA` models in terms of main model fitting. 
```{r}
...
- The only Gibbs sampler step time that should clearly differ between `NNGPblockFixedL` and `NNGPsequenFixedL` is the step updating all $\alpha_{jl}(\bm{s}_{i,o})$'s, which result from whether we adopt our sequential updating method or not. Here, `NNGPsequenFixedL` is mostly more than two times faster than `NNGPblockFixedL` for the posterior sampling step corresponding to $\alpha_{jl}(\bm{s}_{i,o})$'s;
...
- There aren't any significant differences between our 4 models regarding posterior sampling time for the 3 temporal parameters $\psi$, $\Upsilon$, and $\bm{\eta}_t$'s.
...
rm(Diags, Diags.block, Diags.sequen, Diags.sequenVaryLj)
rm(spBFADiags, spBFADiagsLInf)
NKeep <- 5000
...
```
All 6 models' diagnostics and converged posterior deviances are comparable.
...
ggplot(YtestingTempDF) + geom_histogram(aes(x = YtestingTemp), bins = 50) +
  ...
  ggtitle("Actual Distribution of y at All 352 Training Locations and Time 51:60")
...
tempPredMetricMat
```
Temporal prediction results from our models `NNGPsequenFixedL` and `fullGPfixedL` are comparable to their `spBFA` model counterparts. Similar conclusions can be drawn from future-time prediction metrics calculated from temporal prediction objects generated using 3 other random seeds presented below. Future-time prediction performances of `NNGPblockFixedL` and `NNGPsequenVaryLj` can be greatly improved by increasing the number of training time points.
...
```
```{r echo = FALSE}
setwd("fullGPfixedL")#eval = FALSE, 
load("s19toyExNNGPtemppredFixedL.RData")
setwd("../NNGPblockFixedL")
...
tempPredMetricMat
setwd("fullGPfixedL")
load("s27toyExNNGPtemppredFixedL.RData")
setwd("../NNGPblockFixedL")
...
tempPredMetricMat
setwd("fullGPfixedL")
load("s31toyExNNGPtemppredFixedL.RData")
setwd("../NNGPblockFixedL")
...
setwd("../spBFA")
load("s31toyExspBFAL10temppred.RData")
load("s31toyExspBFALInftemppred.RData")
setwd("..")
tempPredMetricMat <- matrix(0, 3, 6, 
                            dimnames = list(c("postMeanMSE", "postMSE", "postVar"),
                                            c("fullGPfixedL", "NNGPblockFixedL", 
...
## Predictions for all 50 Training Time Points at the 9 Testing Locations
...
ggplot(YtestingSpatDF) + geom_histogram(aes(x = YtestingSpat), bins = 50) +
  ...
  theme(axis.ticks.x = element_blank(), axis.ticks.y = element_blank(),
        plot.title = element_text(hjust = 0.5)) +
  ggtitle("Actual Distribution of y at 9 Testing Locations and Time 1:50")
...
spatPredMetricMat
```
All spatial prediction metrics are appropriately small in light of the magnitudes of the actual $y_t(\bm{s}_i)$'s at the 9 testing locations and time $t=1,2,\dots,50$. The relative new-location prediction performances of `NNGPsequenVaryLj` can be significantly improved by increasing the number of training time points.

### Spatial Prediction Time Breakdown for Our 4 Models
```{r}
...
- `alphaKrigTime` corresponding to `NNGPblockFixedL` and `NNGPsequenFixedL` should be close and markedly smaller than that of `fullGPfixedL`. `NNGPsequenVaryLj`'s `alphaKrigTime` should be even smaller;
- `weightsXiLambdaKrigTime` corresponding to `NNGPsequenVaryLj` should be smaller than their counterparts from the other 3 models.

It is worth noting that the breakdown time (in milliseconds) above can be quite different with the same set-up. We present below results from some other spatial prediction instances from the same model fitting objects we have obtained.
```{r eval = FALSE}
setwd("fullGPfixedL")
load("toyExfullGPfixedL.RData")
spatpredFixedL <- predictNewLocFixedL(regFixedL.simu, 9, distOrigNew, distNewNew, 
...
```{r}
setwd("fullGPfixedL")
load("s29toyExNNGPspatpredFixedL.RData")
setwd("../NNGPblockFixedL")
...
#(testingLocNum * trainingT * O) * NKeep 
ylocPredMean <- apply(ylocPred, 1, mean) 
# of length N = testingLocNum x O x trainingT (rowMeans) note that O = 1 here
spatPredMetricMat[1, 1] <- mean((ylocPredMean - YtestingSpat)^2)
diffMat <- sweep(ylocPred, 1, YtestingSpat, "-")
spatPredMetricMat[2, 1] = mean(rowMeans(diffMat^2))
spatPredMetricMat[3, 1] = mean(apply(ylocPred, 1, var))
...
spatPredMetricMat
setwd("fullGPfixedL")
load("s27toyExNNGPspatpredFixedL.RData")
setwd("../NNGPblockFixedL")
...
spatPredMetricMat
setwd("fullGPfixedL")
load("s31toyExNNGPspatpredFixedL.RData")
setwd("../NNGPblockFixedL")
...
spatPredMetricMat
setwd("fullGPfixedL")
load("s19toyExNNGPspatpredFixedL.RData")
setwd("../NNGPblockFixedL")
...
randIndMat
```
The above results suggest that our 3 models `fullGPfixedL`, `NNGPblockFixedL`, and `NNGPsequenFixedL` all lead to completely correct spatial clustering outcomes. `NNGPsequenVaryLj` k-means clustering on three posterior weights matrices formulated from 10, 100, and 1000 selected equally dispersed kept post-burn-in MCMC iterations' parameter estimates (Section 5.2) also leads to highly accurate spatial clustering results (with only `r m * (1 - accuRatioMat[1, 4])`, `r m * (1 - accuRatioMat[2, 4])`, and `r m * (1 - accuRatioMat[3, 4])` out of `r m` testing locations clustered wrongly), as depicted below.
```{r}
...
fittedNNGPsequenVaryLjspatGpDF <- data.frame(x = xcoordTraining, y = ycoordTraining, 
                                             nkeep10 = 
                                               as.factor(fittedClusGpMat.NNGPsequenVaryLj[,1]),
                                             nkeep100 = 
                                               as.factor(fittedClusGpMat.NNGPsequenVaryLj[,2]),
                                             nkeep1000 = 
                                               as.factor(fittedClusGpMat.NNGPsequenVaryLj[,3]))
...
# 5 locations clustered wrongly (0.9857955*352 = 347)
disaDFnkeep10 <- data.frame(x = xcoordTraining[!nkeep10agreements], 
                            y = ycoordTraining[!nkeep10agreements])
nkeep10NNGPsequenVaryLj <- nkeep10NNGPsequenVaryLjBW + 
  geom_point(data = disaDFnkeep10, aes(x = x, y = y), 
             col = "red", fill = NA, shape = 0, size = 7.1)
...
# 4 locations clustered wrongly (0.9886364*352 = 348)
disaDFnkeep100 <- data.frame(x = xcoordTraining[!nkeep100agreements], 
                             y = ycoordTraining[!nkeep100agreements])
nkeep100NNGPsequenVaryLj <- nkeep100NNGPsequenVaryLjBW + 
  geom_point(data = disaDFnkeep100, aes(x = x, y = y), 
             col = "red", fill = NA, shape = 0, size = 7.1)
...
# 4 locations clustered wrongly (0.9886364*352 = 348)
disaDFnkeep1000 <- data.frame(x = xcoordTraining[!nkeep1000agreements], 
                              y = ycoordTraining[!nkeep1000agreements])
nkeep1000NNGPsequenVaryLj <- nkeep1000NNGPsequenVaryLjBW + 
  geom_point(data = disaDFnkeep1000, aes(x = x, y = y), 
             col = "red", fill = NA, shape = 0, size = 7.1)
