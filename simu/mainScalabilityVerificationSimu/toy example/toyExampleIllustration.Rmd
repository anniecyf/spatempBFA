...
tempPredMetricMat
```
Temporal prediction results from our models `NNGPsequenFixedL` and `fullGPfixedL` are comparable to their `spBFA` model counterparts. Similar conclusions can be drawn from future-time prediction metrics calculated from temporal prediction objects generated using 3 other random seeds presented below.
...
spatPredMetricMat
```
All spatial prediction metrics are appropriately small in light of the magnitudes of the actual $y_t(\bm{s}_i)$'s at the 9 testing locations and time $t=1,2,\dots,50$.
