...
```
All our 4 models' diagnostics and converged posterior deviances are comparable their `spBFAL10` counterparts and clearly better than their `spBFALInf` counterparts.
...
tempPredMetricMat
```
Temporal prediction results from all our 4 models are close and significantly better than their `spBFALInf` model counterparts. Among our 4 models and `spBFAL10`, `NNGPsequenFixedL` appears to be the best, followed by `fullGPfixedL`, `NNGPblockFixedL`, `spBFAL10`, and finally `NNGPsequenVaryLj`. Similar conclusions can be drawn from future-time prediction metrics calculated from temporal prediction objects generated using 3 other random seeds, as presented below.
```{r}
setwd("fullGPfixedL")
