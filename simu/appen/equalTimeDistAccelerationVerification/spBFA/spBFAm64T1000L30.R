...
reg.simu <- bfa_sp(Y ~ 0, data = dat, dist = D, time = Time,  K = K, 
                   ...
                   include.space = TRUE,
                   clustering = TRUE) 
save(reg.simu, file="spBFAL30simuT1000M64Iter30000.RData")
Sys.time()
reg.simu$runtime
library(coda)
Diags <- spBFA::diagnostics(reg.simu, diags = c("dic", "dinf", "waic"), keepDeviance = TRUE)
save(Diags, file="spBFAL30simuT1000M64Iter30000Diags.RData")
Deviance <- as.mcmc(Diags$deviance)
#save(Deviance, file = "spBFAL30simuT1000M64Iter30000Deviance.RData")
