...
for(n1 in 1:N1){
  ...
  regVaryLj.simu.sequen <- bfaVaryingLjs(Y ~ 0, data = dat, dist = D, time = Time,  K = K, LjVec = LjVec,
                                         ...
                                         storeWeights = TRUE)  
  save(regVaryLj.simu.sequen, file = paste0("NNGPsequenVaryLj", n1, ".RData"))
  for (nseed in 1:nSeed) {
    ...
    for (n2 in 1:n) {
      ...
      t2 <- Sys.time()
      # save(spatpredVaryLjSequen, file = paste0("NNGPsequenVaryLj", n1, "spatpred", n2, "seed", seed, ".RData"))
      spatpredTimeVec[count] = difftime(t2, t1, units = "secs")
      alphaKrigTimeVec[count] = spatpredVaryLjSequen$alphaKrigTime
      weightsXiLambdaKrigTimeVec[count] = spatpredVaryLjSequen$weightsXiLambdaKrigTime
      count = count + 1
    }
...
save(spatpredTimeVec, file = "NNGPsequenVaryLjspatpredTimeVec.RData")
save(alphaKrigTimeVec, file = "NNGPsequenVaryLjalphaKrigTimeVec.RData")
save(weightsXiLambdaKrigTimeVec, file = "NNGPsequenVaryLjweightsXiLambdaKrigTimeVec.RData")
