...
for(n1 in 1:N1){ 
  ...
  save(regFixedL.simu.sequen, file = paste0("NNGPsequenFixedL", n1, ".RData"))
  for (nseed in 1:nSeed) {
    ...
    for (n2 in 1:n) {
      ...
      t2 <- Sys.time()
      # save(spatpredFixedLsequen, file = paste0("NNGPsequenFixedL", n1, "spatpred", n2, "seed", seed, ".RData"))
      spatpredTimeVec[count] = difftime(t2, t1, units = "secs")
      alphaKrigTimeVec[count] = spatpredFixedLsequen$alphaKrigTime
      weightsXiLambdaKrigTimeVec[count] = spatpredFixedLsequen$weightsXiLambdaKrigTime
      count = count + 1
    }
...
save(spatpredTimeVec, file = "NNGPsequenFixedLspatpredTimeVec.RData")
save(alphaKrigTimeVec, file = "NNGPsequenFixedLalphaKrigTimeVec.RData")
save(weightsXiLambdaKrigTimeVec, file = "NNGPsequenFixedLweightsXiLambdaKrigTimeVec.RData")
save(postGmat, file = "NNGPsequenFixedLpostGmat_spatPredMetricSimu.RData")
save(postMSEmat, file = "NNGPsequenFixedLpostMSEmat_spatPredMetricSimu.RData")
save(postVarMat, file = "NNGPsequenFixedLpostVarMat_spatPredMetricSimu.RData")
