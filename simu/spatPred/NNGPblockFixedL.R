...
for(n1 in 1:N1){ 
  ...
  save(regFixedL.simu.block, file = paste0("NNGPblockFixedL", n1, ".RData"))
  for (nseed in 1:nSeed) {
    ...
    for (n2 in 1:n) {
      ...
      t2 <- Sys.time()
      # save(spatpredFixedLblock, file = paste0("NNGPblockFixedL", n1, "spatpred", n2, "seed", seed, ".RData"))     
      spatpredTimeVec[count] = difftime(t2, t1, units = "secs")
      alphaKrigTimeVec[count] = spatpredFixedLblock$alphaKrigTime
      weightsXiLambdaKrigTimeVec[count] = spatpredFixedLblock$weightsXiLambdaKrigTime
      count = count + 1
    }
...
save(spatpredTimeVec, file = "NNGPblockFixedLspatpredTimeVec.RData")
save(alphaKrigTimeVec, file = "NNGPblockFixedLalphaKrigTimeVec.RData")
save(weightsXiLambdaKrigTimeVec, file = "NNGPblockFixedLweightsXiLambdaKrigTimeVec.RData")
save(postGmat, file = "NNGPblockFixedLpostGmat_spatPredMetricSimu.RData")
save(postMSEmat, file = "NNGPblockFixedLpostMSEmat_spatPredMetricSimu.RData")
save(postVarMat, file = "NNGPblockFixedLpostVarMat_spatPredMetricSimu.RData")
